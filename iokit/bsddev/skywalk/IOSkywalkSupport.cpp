...
IOSKRegionMapper::map( IOSKIndex segIndex, IOSKBuffer * buffer )
{
	...
	if (buffer->fSpec.writable == FALSE) {
...
IOSKRegion::free( void )
{
	...
	if (fArenaHead) {
		...
		SLIST_FOREACH_SAFE(entry, fArenaHead, link, tentry) {
			...
			IOFree(entry, sizeof(ArenaEntry));
		}
		IOFree(fArenaHead, sizeof(ArenaEntry));
	}
	...
	IOSKArena * arena, IOSKOffset regionOffset, IOSKIndex regionIndex )
{
	...
	if (!fArenaHead) {
		fArenaHead = (ArenaHead *)IOMalloc(sizeof(ArenaHead));
	...
	if (fArenaHead) {
		...
		if (dup != NULL) {
			...
			// append a new entry
			ArenaEntry * newEntry = (ArenaEntry *)IOMalloc(sizeof(ArenaEntry));
...
OSDefineMetaClassAndFinalStructors( IOSKMemoryBuffer, IOBufferMemoryDescriptor )
...
{
	bool ok = true;
	IOOptionBits options = (kIOMemoryKernelUserShared | kIODirectionInOut);
	...
	ok = initWithPhysicalMask(inTask, options, capacity, alignment, 0);
	...
	mach_vm_address_t * kvaddr )
{
	...
	if (mb && !mb->initWithSpec(TASK_NULL, capacity, PAGE_SIZE, spec)) {
		mb->release();
		mb = NULL;
	}
	if (!mb) {
	...
	if (kvaddr) {
		*kvaddr = (mach_vm_address_t)(uintptr_t)mb->getBytesNoCopy();
	}
