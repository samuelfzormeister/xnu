...
cuckoo_hashtable_init(void)
{
	...
	ASSERT(cuckoo_tag != NULL);
	cht_group_attr = lck_grp_attr_alloc_init();
	ASSERT(cht_group_attr != NULL);
	cht_lock_attr = lck_attr_alloc_init();
	ASSERT(cht_lock_attr != NULL);
	cht_lock_group = lck_grp_alloc_init("CHT_LOCK", cht_group_attr);
	ASSERT(cht_lock_group != NULL);
}
