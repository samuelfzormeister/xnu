...
static lck_grp_attr_t *headless_lck_grp_attr;
static lck_grp_t *headless_lck_grp;
static lck_attr_t *headless_lck_attr;
static lck_mtx_t *headless_lck_mtx;
...
headless_packet_pool_alloc(boolean_t multi_buflet, unsigned int max_mtu)
{
	...
	if (error != 0) {
		...
		MALLOC(fpp, headless_packet_pool_t, sizeof(*fpp), M_DEVBUF, M_WAITOK | M_ZERO);
		fpp->fpp_pp = pp;
